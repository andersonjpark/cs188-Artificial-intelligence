...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            tempValueForState = util.Counter()
            for curState in states:
                curMax = -9999999
                if self.mdp.isTerminal(curState) is not True:
                    successor = self.mdp.getPossibleActions(curState)
                    for nextState in successor:
                        qValue = self.computeQValueFromValues(curState, nextState)
                        if curMax < qValue:  curMax = qValue
                    tempValueForState[curState] = curMax
            for curState in states:
                self.values[curState] = tempValueForState[curState]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for stateAndProb in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            totalQValue += prob*(reward+discountedValue)
        return totalQValue
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        lenStates = len(self.mdp.getStates())
        states = self.mdp.getStates()

        for iter in range(self.iterations):
            index = iter % lenStates
            curState = states[index]
            curMax = float("-Inf")
            if self.mdp.isTerminal(curState) is not True:
                successor = self.mdp.getPossibleActions(curState)
                for action in successor:
                    qValue = self.computeQValueFromValues(curState, action)
                    if curMax < qValue:  curMax = qValue
                self.values[curState] = curMax
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(tempState):
                ...
                for action in actions:
                    ...
                    qValues.append(qValue)
                self.values[tempState] = max(qValues)
